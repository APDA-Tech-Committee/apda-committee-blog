...
import { getCommittees, type Committee } from '../utils/staticData'
...
onMounted(async () => {
  try {
    ...
    committees.value = response.committees
  } catch (err) {
          ...
          <div class="relative z-10 p-8">
            ...
            <div class="flex items-center justify-between text-sm font-medium text-gray-600 uppercase tracking-wide mb-8">
              <div class="flex items-center gap-2">
                ...
                <span>{{ committee.members ? committee.members.length : 0 }} member{{ committee.members && committee.members.length !== 1 ? 's' : '' }}</span>
              </div>
              <div class="flex items-center gap-2">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
