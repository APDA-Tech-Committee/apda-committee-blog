        ...
        <header class="mb-8">
          ...
          <div class="flex flex-wrap items-center gap-4 text-sm text-text-secondary pb-6">
            <div class="flex items-center gap-2">
              ...
              <span>
                {{ post.author.name }}
                <span v-if="post.author.position" >
...
const formatContent = (content: string) => {
  ...
  let listMatches = formatted.match(/(?:^|\n)(?:\d+\. .*(?:\n|$))+/gm);
  if (listMatches) {
    listMatches.forEach(match => {
      const listContent = match.replace(/^\d+\. (.*?)$/gm, '<li>$1</li>');
      formatted = formatted.replace(match, `<ol class="list-decimal pl-6 my-4">${listContent}</ol>`);
    });
  }
